...
class AiSearchRerankModel(RerankModel):
    ...
    def _invoke(
        ...
        try:
            ...
            rerank_documents, total_tokens = self._process_response(
                response, docs, score_threshold or self.DEFAULT_SCORE_THRESHOLD
            )
    ...
    def _get_region_from_credentials(self, credentials: dict) -> str:
        """
        Get region configuration from credentials
        ...
        if not region:
            region = self.DEFAULT_API_REGION
            logger.info(f"Using default region: {region}")

        # Validate if region is in supported list (optional, provides better error message)
        if region not in self.SUPPORTED_REGIONS:
            logger.warning(f"Region {region} is not in supported list, but will try to use it")
