...
class AiSearchLargeLanguageModel(LargeLanguageModel):
    ...
    def _process_stream_response(
        ...
        for index, event in enumerate(response):
            try:
                # Process response data format
                response_data = None

                # Check if it's a dictionary type (contains data field)
                if isinstance(event, dict) and 'data' in event:
                    try:
                        # Parse JSON in data field
                        data_content = event['data']
                        if isinstance(data_content, str):
                            parsed_data = json.loads(data_content)
                            response_data = parsed_data.get('Response')
                        else:
                            response_data = data_content.get('Response') if isinstance(data_content,
                                                                                       dict) else data_content
                    ...
                    try:
                        if isinstance(data_attr, str):
                            ...
                            response_data = data_attr.get('Response') if isinstance(data_attr, dict) else data_attr
                    except json.JSONDecodeError:
                        continue
                else:
                    response_data = event
                ...
                content, is_reasoning = self._wrap_thinking_by_reasoning_content(message, is_reasoning)
                ...
                if tool_calls_data:
                    tool_calls = self._extract_response_tool_calls(tool_calls_data)
    ...
    def _create_correct_usage(self, prompt_tokens: int, completion_tokens: int) -> dict:
        """
        Create usage structure compliant with Dify plugin requirements
        """
        latency = time.perf_counter() - self.started_at

        # Calculate pricing
        input_unit_price = Decimal("0.002")
        output_unit_price = Decimal("0.008")

        prompt_price = (input_unit_price * prompt_tokens) / Decimal("1000")
        completion_price = (output_unit_price * completion_tokens) / Decimal("1000")
        total_price = prompt_price + completion_price
        total_tokens = prompt_tokens + completion_tokens

        return {
            "prompt_tokens": prompt_tokens,
            "prompt_unit_price": float(input_unit_price),
            "prompt_price_unit": Decimal("1"),
            "prompt_price": float(prompt_price),

            "completion_tokens": completion_tokens,
            "completion_unit_price": float(output_unit_price),
